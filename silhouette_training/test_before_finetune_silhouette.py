...
def test_model(model, test_loader, device):
    ...
    if path.exists("log_test_before_finetune_silhouette"):
        f_log = open("log_test_before_finetune_silhouette/test_before_finetune.txt", "a+")
    else:
        os.mkdir("log_test_before_finetune_silhouette")
        f_log = open("log_test_before_finetune_silhouette/test_before_finetune.txt", "a+")
    ...
    result = f"{total_acc:<30}" + 'time_elapsed: {:4f} Acc: {:.4f} Acc_top5: {:.4f}\n'.format(time_elapsed, total_acc,
                                                                                              total_acc_top5)
    f_log.write(result)
    for cls in classes:
