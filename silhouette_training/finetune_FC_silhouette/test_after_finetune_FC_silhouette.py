...
def test_model(model, test_loader, device, model_id):
    ...
    if path.exists("log_test_after_FC_finetune_silhouette"):
        f_log = open("log_test_after_finetune_FC_silhouette/test_after_finetune_model" + str(model_id) + ".txt", "a+")
    else:
        os.mkdir("log_test_after_finetune_FC_silhouette")
        f_log = open("log_test_after_finetune_FC_silhouette/test_after_finetune_model" + str(model_id) + ".txt", "a+")
