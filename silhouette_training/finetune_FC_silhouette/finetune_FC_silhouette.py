...
def finetune_model_FC_silhouette(model, trainloader, criterion, optimizer, num_epochs, device, batch_size):
    since = time.time()

    log_frequency = 16  # batch size is 256, write to log1 every 10 batches

    model_path = "finetune_FC_model_silhouette"
    ...
    if os.path.exists("log_finetune_FC_silhouette"):
        running_log = open("log_finetune_FC_silhouette/train_finetune_running_loss.txt", "w+")
    else:
        os.mkdir("log_finetune_FC_silhouette")
        running_log = open("log_finetune_FC_silhouette/train_finetune_running_loss.txt", "w+")
    ...
    if os.path.exists("log_finetune_FC_silhouette"):
        epoch_log = open("log_finetune_FC_silhouette/train_finetune_epoch_loss.txt", "w+")
    ...
    for epoch in range(num_epochs):
        ...
        if os.path.exists("log_finetune_FC_silhouette"):
            epoch_log = open("log_finetune_FC_silhouette/train_finetune_epoch_loss.txt", "a+")
        ...
        for data_index, (inputs, labels) in enumerate(trainloader):
            running_log = open("log_finetune_FC_silhouette/train_finetune_running_loss.txt", "a+")
