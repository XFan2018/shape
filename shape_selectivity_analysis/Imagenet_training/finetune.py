...
def train_model(model, trainloaders, criterion, optimizer, num_epochs, device, batch_size):
    ...
    log_frequency = 2  # batch size is 256, write to log1 every 10 batches

    model_path = "finetune_model"
    model_weight_path = "finetune_model_weight"
    ...
    if os.path.exists("log1"):
        running_log = open("log1/train_finetune_running_loss.txt", "w+")
    else:
        os.mkdir("log1")
        running_log = open("log1/train_finetune_running_loss.txt", "w+")
    ...
    if os.path.exists("log1"):
        epoch_log = open("log1/train_finetune_epoch_loss.txt", "w+")
    ...
    for epoch in range(num_epochs):
        ...
        if os.path.exists("log1"):
            epoch_log = open("log1/train_finetune_epoch_loss.txt", "a+")
        ...
        for data_index, (inputs, labels) in enumerate(trainloaders):
            running_log = open("log1/train_finetune_running_loss.txt", "a+")
        ...
        if os.path.exists(model_weight_path):
            torch.save(model.state_dict(), model_weight_path + "/param.pkl")
        else:
