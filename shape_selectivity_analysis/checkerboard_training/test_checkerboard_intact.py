...
def test_model_intact(model, test_loader, log_path, device, model_path, model_number):
    since = time.time()
    if path.exists(log_path):
        f_log = open(log_path + "/test_after_finetune.txt", "a+")
    else:
        os.mkdir(log_path)
        f_log = open(log_path + "/test_after_finetune.txt", "a+")
    ...
    result = f"{tag:<30}" + 'time_elapsed: {:4f} Acc: {:.4f} Acc_top5: {:.4f} confidence: {:.4f}\n'.format(time_elapsed, total_acc,
                                                                                        total_acc_top5, confidence_score)
    f_log.write(result)
    f_log.close()
